...
- Below the list, the income section: a 總預算 header row, then the income list (labels in B, 幣別 USD/TWD in C, amounts in D): 沛還, 薪水, plus ad-hoc income rows (e.g. 多一個月薪水) — manage these with set_income, which upserts by 項目 and keeps the SUMIFs covering every row. The list ends at 月美金餘額 / 月新臺幣餘額 (THIS month's 收入−支出 per currency, from the 銀行餘額 block), then 透支沖銷 (automatic all-or-nothing write-off: =IF(總新臺幣餘額 >= 上月透支, 上月透支, 0) — when the bank can cover the carried overdraft it is settled from savings), then 月剩餘 (= 月美金餘額*GOOGLEFINANCE USDTWD + 月新臺幣餘額 + 透支沖銷 — the month's combined remainder in TWD; because 透支沖銷 adds a settled carry back, only FRESH overspending rolls to next month). The old 剩餘, 美金支付 and 新臺幣支付 rows are DEPRECATED and removed by migration.
