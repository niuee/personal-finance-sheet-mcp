...
- Further down, the 銀行餘額 block tracks the month's per-currency money flow (labels in column B, values in column D): 本月美金收入 / 本月美金支出 / 本月初美金餘額 / 本月底美金餘額, then 本月新臺幣收入 / 本月新臺幣支出 / 午餐超支或回補 / 本月初新臺幣餘額 / 保守預計本月底新臺幣餘額 / 本月底新臺幣餘額. 收入 cells = SUMIF over the income list's 幣別 column; 本月美金支出 = SUMIF of the expense 支付幣別 for USD summing column D; 本月新臺幣支出 = SUMIF for TWD summing column E, plus the transfer block's N總和. Both 支出 SUMIFs span the FULL expense window INCLUDING the 上月…透支 carry row(s) — deliberate: Vincent counts the carried shortfall as an outflow that must be covered out of this month's money in the 收支狀況 budget view. The 本月底…餘額 / 保守預計 formulas then ADD THE CARRY BACK (+ the 上月美金透支 D cell / + the 上月新臺幣透支 E cell): with the openings chained, the shortfall already arrives via the negative opening balance, so without the add-back the balance ledger would subtract it twice. Keep both halves — do not "simplify" away either the SUMIF window or the +carry terms. BOTH currencies chain since the 調整 layout: 本月初美金餘額 / 本月初新臺幣餘額 point at the previous month's 調整後 rows (start_month rewires them; 本月初美金餘額 is no longer a hardcoded 0). 本月底美金餘額 = 本月初 + 收入 − 支出 + K總和 (USD received); 本月底新臺幣餘額 = 本月初 + 收入 − 支出 − I總和 (NTD sent) + 午餐超支或回補; 保守預計本月底新臺幣餘額 is the same but counts 午餐超支或回補 only when it is negative (a lunch overspend). Below each 本月底 row sits 調整後本月底美金餘額 / 調整後的本月底新臺幣餘額 (6月 titles the USD one 調整後的… too) = the raw 本月底 + that currency's …餘額調整 cell from the 帳戶實際數字對應 block — the bank view absorbs the same reconciliation delta.
